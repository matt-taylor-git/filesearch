...
class TestGetFileInfo:
    ...
    def test_get_file_info_for_directory(self, temp_dir):
        """Test getting file info for a directory."""
        info = get_file_info(temp_dir)
        
        assert info['path'] == str(temp_dir.resolve())
...
class TestSafeOpen:
    ...
    def test_safe_open_directory(self, temp_dir):
...
class TestOpenContainingFolder:
    ...
    def test_open_containing_folder_linux_file(self, mock_run, mock_system, temp_file):
        ...
        assert result is True
        mock_run.assert_called_once_with(["xdg-open", str(temp_file.parent)], check=True)
    
    @patch('platform.system')
    @patch('subprocess.run')
    def test_open_containing_folder_linux_directory(self, mock_run, mock_system, temp_dir):
        """Test opening directory on Linux."""
        mock_system.return_value = "Linux"
        mock_run.return_value = Mock()
        
        result = open_containing_folder(temp_dir)
        
        assert result is True
        mock_run.assert_called_once_with(["xdg-open", str(temp_dir)], check=True)
    ...
    @patch('platform.system')
    def test_open_containing_folder_windows_directory(self, mock_system, temp_dir):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert result is True
            mock_run.assert_called_once_with(["explorer", str(temp_dir)], check=True)
...
class TestConvenienceFunctions:
    ...
    def test_is_directory_with_directory(self, temp_dir):
